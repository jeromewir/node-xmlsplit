...
XmlSplit.prototype._transform = function(chunk, encoding, done) {
  ...
  if (!this._header) {
    ...
    var res = this._data.match(/<(\w+|\/\w+)[>\s]/mg);
    if (res !== null) {
      var r;
      var i = 0;
      var index = -1;
      ...
      while (res[i]) {
        r = res[i];
        r = r.replace(/<\/?|>/g, '').trim();
        if (r === this._tagName) break;
        if ((index = this._headerEndTag.indexOf(r)) >= 0) {
          this._headerEndTag.splice(index, 1);
        } else {
          this._headerEndTag.push(r);
        }
        ++i;
      }
      this._headerEndTag = this._headerEndTag.reverse().map(h => `</${h}>`).join('');
      res = (/<(\w+)[>\s]/mg).exec(this._data);
